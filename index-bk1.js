...
async function initializeData() {
  ...
  for (const [parkKey, park] of Object.entries(CONFIG.PARKS)) {
    const schedule = await fetchParkSchedule(park.id);
    if (schedule.open) initialData.parks[park.id].hours.open = schedule.open;
    if (schedule.close) initialData.parks[park.id].hours.close = schedule.close;
    console.log(`Fetched initial schedule for ${parkKey}: ${schedule.open} - ${schedule.close}`);
  }
...
// Fetch schedule (hours) for a specific park.
// Uses the earliest openingTime across OPERATING and TICKETED_EVENT entries (e.g. Early Entry)
// and the closingTime from the OPERATING entry.
async function fetchParkSchedule(parkId) {
  try {
    ...
    const todayEntries = (data.schedule || []).filter(
      s => s.date === today && (s.type === 'OPERATING' || s.type === 'TICKETED_EVENT')
    );

    if (todayEntries.length === 0) return { open: null, close: null };

    // Earliest open across all relevant entries (covers Early Entry / TICKETED_EVENT)
    const earliestOpen = todayEntries
      .map(s => s.openingTime)
      .filter(Boolean)
      .sort()[0];

    // Closing time comes from the OPERATING entry
    const operatingEntry = todayEntries.find(s => s.type === 'OPERATING');
    const closingTime = operatingEntry?.closingTime || null;

    return { open: earliestOpen || null, close: closingTime };
  } catch (error) {
...
// Fetch and store schedules for all parks
async function fetchAllParkSchedules() {
  console.log('Fetching park schedules...');
  const storedData = await loadData();
  for (const [parkKey, park] of Object.entries(CONFIG.PARKS)) {
    const schedule = await fetchParkSchedule(park.id);
    if (schedule.open) storedData.parks[park.id].hours.open = schedule.open;
    if (schedule.close) storedData.parks[park.id].hours.close = schedule.close;
    console.log(`Fetched schedule for ${parkKey}: ${schedule.open} - ${schedule.close}`);
  }
...
function scheduleDailyHoursUpdate() {
  ...
  console.log(`Next schedule fetch in ${Math.round(msUntil2AM / 60000)} minutes (2:00 AM ET)`);
  setTimeout(async () => {
...
// Check if current time is within park hours
function isWithinParkHours(parkData) {
  if (!parkData.hours.open || !parkData.hours.close) {
    return false;
  }
  
  const now = new Date();
  const openTime = new Date(parkData.hours.open);
  const closeTime = new Date(parkData.hours.close);
...
// Format hours for pinned message
function formatHours(open, close) {
  if (!open || !close) {
    return 'Closed';
  }
  const openTime = new Date(open).toLocaleTimeString('en-US', { 
    hour: '2-digit', 
    minute: '2-digit',
    hour12: false,
    timeZone: 'America/New_York'
  });
  const closeTime = new Date(close).toLocaleTimeString('en-US', { 
    hour: '2-digit', 
    minute: '2-digit',
    hour12: false,
    timeZone: 'America/New_York'
...
function buildPinnedMessage(parkKey, parkData, attractionsList) {
  ...
  const hours = formatHours(parkData.hours.open, parkData.hours.close);
